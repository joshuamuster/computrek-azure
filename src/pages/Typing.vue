<template>
  <div class="typing-hub" :class="{ show: isVisible }">
    ...
    <!-- ── Foundations section ───────────────────────────────────────────────── -->
    <div v-if="foundationLessons.length" ref="foundationListRef" class="lesson-section tutorial-section">
      <div class="section-header">
        <span class="section-title">Foundations</span>
        <span class="section-count">{{ foundationLessons.length }} lessons</span>
      </div>
      ...
      <div class="lesson-list">
        <div
          ...
          :class="{ 'lesson-locked': !isAccessible(lesson), 'lesson-item--checkpoint': isCheckpointLesson(lesson) }"
          @click="isAccessible(lesson) ? router.push(`/typing/lesson/${lesson.id}`) : null"
        >
          <div class="lesson-num">{{ index + 1 }}</div>
          <div class="lesson-body">
            <div class="lesson-name">
              {{ lesson.title }}
              <span v-if="isCheckpointLesson(lesson)" class="checkpoint-flag">🎯 Checkpoint</span>
            </div>
          ...
          <!-- Sealed-but-passed lessons show ✓ (record preserved); not-yet-reached show 🔒 -->
          <div class="lesson-arrow">{{ !isAccessible(lesson) ? (hasPassedLesson(lesson) ? '✓' : '🔒') : (progressLoading ? '…' : (hasPassedLesson(lesson) ? '✓' : '›')) }}</div>
        </div>
      </div>
    </div>
    ...
    <div v-if="test1Lesson" class="test-checkpoint">
      ...
      <p class="checkpoint-desc">Complete during class to unlock Core Skills.</p>
      <div
    ...
    <!-- ── Core Skills section ──────────────────────────────────────────────── -->
    <div ref="coreSkillsListRef" class="lesson-section">
      <div class="section-header">
        <span class="section-title">Core Skills</span>
        <span v-if="coreSkillLessons.length" class="section-count">{{ coreSkillLessons.length }} lessons</span>
        <span v-if="test1Lesson && attemptCount('test-1') === 0" class="section-gate-badge">🔒 Complete Test 1 to unlock</span>
      </div>

      <div v-if="isLoading" class="lesson-loading">Loading lessons…</div>

      <div v-else-if="coreSkillLessons.length === 0" class="lesson-empty">
      ...
      <div v-else class="lesson-list">
        <div
          ...
          @click="isAccessible(lesson) ? router.push(`/typing/lesson/${lesson.id}`) : null"
        >
          <div class="lesson-num">{{ foundationLessons.length + index + 1 }}</div>
          <div class="lesson-body">
            <div class="lesson-name">
              {{ lesson.title }}
              <span v-if="isCheckpointLesson(lesson)" class="checkpoint-flag">🎯 Checkpoint</span>
          ...
          <div class="lesson-arrow">{{ !isAccessible(lesson) ? (hasPassedLesson(lesson) ? '✓' : '🔒') : (hasPassedLesson(lesson) ? '✓' : '›') }}</div>
        </div>
      </div>
    </div>
    ...
    <div v-if="test2Lesson" class="test-checkpoint">
      ...
      <p class="checkpoint-desc">Complete during class to unlock Advanced Missions.</p>
      <div
        class="lesson-item test-item checkpoint-item"
        :class="{
    ...
    <div ref="advancedListRef" class="lesson-section star-trek-section">
      <div class="section-header">
        <span class="section-title">Advanced Missions</span>
        <span v-if="advancedLessons.length" class="section-count">{{ advancedLessons.length }} missions</span>
        <span v-if="test2Lesson && attemptCount('test-2') === 0" class="section-gate-badge">🔒 Complete Test 2 to unlock</span>
      </div>

      <div v-if="isLoading" class="lesson-loading">Loading missions…</div>

      <div v-else-if="advancedLessons.length === 0" class="lesson-empty">
      ...
      <div v-else class="lesson-list">
        <div
          ...
          @click="isAccessible(lesson) ? router.push(`/typing/lesson/${lesson.id}`) : null"
        >
          <div class="lesson-num">{{ foundationLessons.length + coreSkillLessons.length + index + 1 }}</div>
          <div class="lesson-body">
            <div class="lesson-name">
              {{ lesson.title }}
              <span v-if="isCheckpointLesson(lesson)" class="checkpoint-flag">🎯 Checkpoint</span>
            ...
            <div class="lesson-sub">
              ...
              <span v-else class="lesson-keys">Full keyboard · {{ lesson.goalWpm }}+ WPM</span>
            </div>
          </div>
          <div class="lesson-arrow">{{ !isAccessible(lesson) ? (hasPassedLesson(lesson) ? '✓' : '🔒') : (hasPassedLesson(lesson) ? '✓' : '›') }}</div>
        </div>
    ...
    <!-- ── Test 3 checkpoint (Semester Exam) ────────────────────────────────── -->
    <div v-if="test3Lesson" class="test-checkpoint test-final">
      ...
      <p class="checkpoint-desc">The final typing exam. Complete during class to earn your Typing Lab certification.</p>
      <div
        class="lesson-item test-item checkpoint-item"
        :class="{
    ...
    <div v-if="expertLessons.length || isLoading" ref="expertListRef" class="lesson-section expert-section">
      <div class="section-header">
        <span class="section-title">Expert Skills</span>
        <span v-if="expertLessons.length" class="section-count">{{ expertLessons.length }} lessons</span>
        <span v-if="test3Lesson && attemptCount('test-3') === 0" class="section-gate-badge">🔒 Complete Test 3 to unlock</span>
      </div>

      <div v-if="isLoading" class="lesson-loading">Loading lessons…</div>

      <div v-else-if="expertLessons.length === 0" class="lesson-empty">
        Expert lessons coming soon.
      </div>
      ...
      <div v-else class="lesson-list">
        <div
          ...
          :key="lesson.id"
          class="lesson-item expert-item"
          :class="{ 'lesson-locked': !isAccessible(lesson), 'lesson-item--checkpoint': isCheckpointLesson(lesson) }"
          @click="isAccessible(lesson) ? router.push(`/typing/lesson/${lesson.id}`) : null"
        >
          <div class="lesson-num">{{ foundationLessons.length + coreSkillLessons.length + advancedLessons.length + index + 1 }}</div>
          <div class="lesson-body">
            <div class="lesson-name">
              {{ lesson.title }}
              <span v-if="isCheckpointLesson(lesson)" class="checkpoint-flag">🎯 Checkpoint</span>
...
const foundationListRef  = ref<HTMLElement | null>(null)
const coreSkillsListRef  = ref<HTMLElement | null>(null)
...
const { isLoading: progressLoading, fetchProgress, hasPassedLesson,
        computeUnlocked, isSectionLocked, attemptCount }             = useStudentProgress()
...
/**
 * A lesson is accessible when it is both:
 *   1. Unlocked in the sequential chain (all prior lessons cleared), AND
 *   2. Not sealed by a completed section test (taken = locked for replay).
 */
const isAccessible = (lesson: TypingLesson): boolean =>
  unlockedIds.value.has(lesson.id) && !isSectionLocked(lesson)

/**
 * Mid-section "Checkpoint: ___" lessons — harder milestone lessons (raised WPM
 * + accuracy targets) that sit inline in the regular chain rather than as
 * scheduled class-wide tests. Flagged by title so they stand out in the list
 * and students can see them coming.
 */
const isCheckpointLesson = (lesson: TypingLesson): boolean =>
  lesson.title.startsWith('Checkpoint:')
...
.expert-section .section-title {
  color: rgba(180, 120, 255, 0.85); /* Purple — distinct from the other sections */
}

.expert-item {
  border-color: rgba(160, 100, 255, 0.13) !important;
}
.expert-item:hover {
  border-color: rgba(160, 100, 255, 0.4) !important;
  background:   rgba(20, 0, 50, 0.5) !important;
}
...
/* ── Inline checkpoint lessons (harder milestone lessons within a section) ── */
.lesson-item.lesson-item--checkpoint {
  border-color: rgba(255, 200, 60, 0.3) !important;
  background:   rgba(255, 200, 60, 0.06) !important;
}
.lesson-item.lesson-item--checkpoint:hover {
  border-color: rgba(255, 200, 60, 0.55) !important;
  background:   rgba(60, 45, 0, 0.45) !important;
}
.lesson-item.lesson-item--checkpoint .lesson-num {
  background:   rgba(255, 200, 60, 0.2);
  border-color: rgba(255, 200, 60, 0.6);
  color:        rgba(255, 224, 140, 1);
}
.checkpoint-flag {
