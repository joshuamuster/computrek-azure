...
import { collection, query, where, doc, getDoc, getDocs, onSnapshot } from '@/data/db'
import { db } from '@/firebase'
import { useAuth } from '@/composables/useAuth.js'
import { useSeatingChart, PRESET_LAYOUTS, buildGridItems, buildGridStyle, type SeatStudent } from '@/composables/useSeatingChart'
...
import { useConductLog, CONDUCT_REASONS, todayISO, type ConductReasonKey } from '@/composables/useConductLog'
...
// only to pick a sensible default when this page first loads. Choosing a
// different period here must NOT change the HUD; see viewPeriodId below.
const { selectedPeriodId: activePeriodId } = usePeriodSelector()

// Local "viewing" period for this page — controls which seating chart
// displays/saves. Defaults to whichever period is "Active" on navigation.
...
watch(
  ...
  async (email) => {
    ...
    if (isStaff.value) {
      ...
      teacherDisplayName.value = (userInfo.value as any)?.displayName ?? email
    } else {
      // Admin emulating — fetch emulated teacher's record
      try {
        const snap = await getDoc(doc(db, 'approvedUsers', email))
        if (snap.exists()) {
          pids = snap.data().periodIds ?? []
          teacherDisplayName.value = snap.data().displayName ?? email
        }
      } catch (e) {
        console.warn('[AdminSeatingChart] could not fetch teacher record:', e)
        teacherDisplayName.value = email
      }
    }
    teacherPeriodIds.value = pids
    // If the currently viewed period isn't valid for this teacher, clear it
    // so the seating area stays blank rather than loading the wrong chart.
    // Do NOT auto-select pids[0] — let the existing selection stand.
    if (viewPeriodId.value && !pids.includes(viewPeriodId.value)) {
...
// Re-fetch whenever the viewed period or teacher changes
watch(
  [viewPeriodId, resolvedTeacherEmail],
  async ([pid, email]) => {
    if (pid && email) {
...
const save = async () => {
  if (!viewPeriodId.value || !resolvedTeacherEmail.value) return
  await saveSeatingChart(viewPeriodId.value, resolvedTeacherEmail.value)
...
const rotateRows = () => {
  ...
  const newSeats: Record<string, SeatStudent> = {}
  for (const [key, student] of Object.entries(seats.value)) {
    const match = key.match(/^r(\d+)c(\d+)$/)
    if (!match) continue
    const row    = parseInt(match[1])
    const col    = parseInt(match[2])
    const newRow = (row - 1 + rows) % rows   // shift up; front row wraps to back
    newSeats[`r${newRow}c${col}`] = student
  }
...
// ─────────────────────────────────────────────────────────────────────────────
// Conduct mode — multi-select + bulk action
// ─────────────────────────────────────────────────────────────────────────────

const { logEntryBulk, closePeriod: runClosePeriod, saving: conductSaving } = useConductLog()

const isConduct       = ref(false)
const selectedUids    = ref<Set<string>>(new Set())

// Conduct form state
const conductDir      = ref<'+1' | '-1' | null>(null)
const conductReason   = ref<ConductReasonKey | null>(null)
const conductNote     = ref('')
const conductDate     = ref(todayISO())
const conductSaved    = ref(false)

const enterConduct = () => {
  isConduct.value    = true
  selectedUids.value = new Set()
  conductDir.value   = null
  conductReason.value = null
  conductNote.value  = ''
  conductDate.value  = todayISO()
}

const exitConduct = () => {
  isConduct.value     = false
  selectedUids.value  = new Set()
  conductDir.value    = null
  conductReason.value = null
  conductNote.value   = ''
  conductSaved.value  = false
}

const toggleSeatSelection = (uid: string) => {
...
const handleSeatClick = (student: SeatStudent) => {
  if (isConduct.value) {
    toggleSeatSelection(student.uid)
  } else {
    openSlideOver(student)
  }
}

const conductCanSubmit = computed(() =>
  conductDir.value !== null && conductReason.value !== null && selectedUids.value.size > 0,
)

const submitConduct = async () => {
  if (!conductCanSubmit.value || !viewPeriodId.value || !resolvedTeacherEmail.value) return

  const delta    = conductDir.value === '+1' ? 1 : -1
  const type     = delta > 0 ? 'positive' as const : 'negative' as const
  const selected = periodStudents.value.filter(s => selectedUids.value.has(s.uid))

  try {
    await logEntryBulk(selected, {
      teacherEmail: resolvedTeacherEmail.value,
      periodId:     viewPeriodId.value,
      type,
      reason:       conductReason.value!,
      scoreDelta:   delta,
      note:         conductNote.value.trim(),
      date:         conductDate.value,
    })
    conductSaved.value = true
    setTimeout(() => exitConduct(), 1400)
  } catch (e) {
    console.error('[AdminSeatingChart] submitConduct error:', e)
  }
}
...
const closeResult       = ref<{ created: number; skipped: number } | null>(null)
const closeResultTimer  = ref<ReturnType<typeof setTimeout> | null>(null)

const confirmClosePeriod = () => { closingPeriod.value = true }
const cancelClosePeriod  = () => { closingPeriod.value = false }

const executeClosePeriod = async () => {
  if (!viewPeriodId.value || !resolvedTeacherEmail.value) return
  closingPeriod.value = false
  try {
    const result = await runClosePeriod(
      periodStudents.value,
      viewPeriodId.value,
      resolvedTeacherEmail.value,
    )
    closeResult.value = result
    if (closeResultTimer.value) clearTimeout(closeResultTimer.value)
    closeResultTimer.value = setTimeout(() => { closeResult.value = null }, 3500)
  } catch (e) {
    console.error('[AdminSeatingChart] closePeriod error:', e)
  }
}
</script>
...
<template>
  <section class="adventure-page">
    <div class="lcars-text-bar"><span>Workstation Assignments{{ viewPeriodId ? ': ' + periodLabel(viewPeriodId as string) : '' }}</span></div>
    ...
    <template v-else>
      ...
      <div class="seating-controls">
        ...
        <div class="period-tabs">
          <button
            ...
            :key="pid"
            class="period-tab"
            :class="{ 'is-active': viewPeriodId === pid }"
            @click="viewPeriodId = pid"
        ...
        <!-- Edit / Rotate / Conduct / Close Period / Save -->
        <div class="chart-actions">

          <!-- Locked mode: CLOSE PERIOD + CONDUCT + ROTATE + EDIT -->
          <template v-if="!isEditing && !isRotating && !isConduct">

            <!-- Close Period inline confirm flow -->
            <template v-if="closeResult">
              <span class="close-result">✓ {{ closeResult.created }} LOGGED, {{ closeResult.skipped }} SKIPPED</span>
            </template>
            <template v-else-if="closingPeriod">
              <span class="close-period-confirm">LOG {{ periodStudents.length }} CADETS?</span>
              <button class="cancel-btn" @click="cancelClosePeriod">NO</button>
              <button class="close-period-go-btn" :disabled="conductSaving" @click="executeClosePeriod">
                <span v-if="conductSaving">…</span>
            ...
            <template v-else>
              <button
                ...
                :disabled="!viewPeriodId || periodStudents.length === 0"
                title="Auto-log a 'no change' entry for all cadets without an entry today"
            ...
            <button
              class="conduct-btn"
              :disabled="!viewPeriodId || periodStudents.length === 0"
              title="Select multiple cadets to log a conduct entry"
              @click="enterConduct"
            >CONDUCT</button>
            <button
              class="rotate-btn"
              :disabled="!viewPeriodId"
              title="Shift every row one closer to the front; front row wraps to back"
              @click="rotateRows"
            >↑ ROTATE ROWS</button>
            <button class="edit-btn" :disabled="!viewPeriodId" @click="enterEdit">EDIT</button>
          </template>

          <!-- Conduct mode: selection count + CANCEL + LOG -->
          <template v-else-if="isConduct">
            <span class="conduct-count">{{ selectedUids.size }} SELECTED</span>
            <button class="cancel-btn" @click="exitConduct">CANCEL</button>
            <button
              class="save-btn"
              :class="{ 'is-saved': conductSaved }"
              :disabled="!conductCanSubmit || conductSaving"
              @click="submitConduct"
            >
              <span v-if="conductSaved">✓ LOGGED</span>
              <span v-else-if="conductSaving">…</span>
              <span v-else>LOG {{ selectedUids.size > 0 ? selectedUids.size : '' }}</span>
            </button>
          </template>

          <!-- Rotate preview mode: CANCEL + SAVE -->
          <template v-else-if="isRotating">
            <span class="rotate-label">ROTATION PREVIEW</span>
            <button class="cancel-btn" @click="cancelRotate">CANCEL</button>
            <button
              class="save-btn"
              :class="{ 'is-saving': saving, 'is-saved': savedFlash }"
              :disabled="saving || !viewPeriodId"
              @click="saveRotate"
            >
      ...
      <!-- ── No period selected ───────────────────────────────────────────────── -->
      <div v-if="!viewPeriodId" class="no-period-msg">
      ...
      <div v-else class="seating-layout">
        ...
        <div class="seating-area">
            ...
            <template v-for="item in gridItems" :key="item.key">
              ...
              <div
                ...
                :class="{
                  ...
                  'is-ineligible': !!item.student && isIneligible(item.student.uid),
                  'is-drag-over':  isEditing && dragOverSeat === item.key,
                  'is-locked':     !isEditing && !isConduct,
                  'is-conduct':    isConduct && !!item.student,
                  'is-selected':   isConduct && !!item.student && selectedUids.has(item.student.uid),
                }"
                :draggable="isEditing && !!item.student"
                :title="item.student ? (isConduct ? 'Select ' + item.student.displayName : 'View ' + item.student.displayName) : undefined"
                @click="item.student ? handleSeatClick(item.student) : undefined"
                @dragstart="isEditing && item.student ? onSeatDragStart($event, item.row, item.col, item.student) : undefined"
                @dragover="isEditing ? onSeatDragOver($event, item.row, item.col) : undefined"
                @dragleave="isEditing ? onSeatDragLeave() : undefined"
                @drop="isEditing ? onSeatDrop($event, item.row, item.col) : undefined"
              >
                <template v-if="item.student">
                  <span class="seat-name">
                    <span class="seat-name__last">{{ item.student.displayName.split(' ').slice(1).join(' ') || item.student.displayName }}</span>
                    <span class="seat-name__first">{{ item.student.displayName.split(' ')[0] }}</span>
                  </span>

                  <!-- Quarter grade badge -->
                  <div class="seat-grade"
                    v-if="studentGradeMap.get(item.student.uid) != null"
                    :style="{ color: gradeColor(studentGradeMap.get(item.student.uid)!) }"
                    title="Current quarter grade"
                  >{{ studentGradeMap.get(item.student.uid) }}%</div>
                  <div class="seat-grade seat-grade--none" v-else title="No graded work yet">—</div>

                  <button v-if="isEditing" class="seat-clear" title="Remove from seat" @click.stop="clearSeat(item.row, item.col)">✕</button>
                  <!-- Conduct score pips -->
                  <div class="seat-pips" :title="pipLabel(studentScoreMap.get(item.student.uid)?.score ?? 4)">
                    <span
                      v-for="i in 4"
                      :key="i"
                      class="seat-pip"
                      :style="pipFilled(studentScoreMap.get(item.student.uid)?.score ?? 4, i)
                        ? { background: pipColor(studentScoreMap.get(item.student.uid)?.score ?? 4) }
                        : {}"
                    ></span>
                  </div>
                </template>
                <template v-else>
                  <span class="seat-vacant">{{ `${item.row + 1}-${item.col + 1}` }}</span>
                </template>
              </div>
            </template>
          </div>
          ...
          <div class="front-banner front-banner--bottom">▼ FRONT OF ROOM</div>
        ...
        <!-- Roster sidebar / Conduct action panel -->
        <div
          class="seating-sidebar"
          :class="{ 'is-drag-over': isEditing && dragOverRoster, 'is-conduct-mode': isConduct }"
        ...
        >
          ...
          <!-- ── Conduct action panel ──────────────────────────────── -->
          <template v-if="isConduct">
            <div class="sidebar-header">
              <span class="sidebar-title">CONDUCT</span>
              <span class="sidebar-period" v-if="viewPeriodId">{{ periodLabel(viewPeriodId) }}</span>
            </div>

            <div v-if="selectedUids.size === 0" class="conduct-empty-hint">
              Click cadets on the chart to select them.
            </div>

            <div v-else class="conduct-form">
              <div class="conduct-selected-count">{{ selectedUids.size }} cadet{{ selectedUids.size !== 1 ? 's' : '' }} selected</div>

              <!-- Direction -->
              <div class="conduct-dir-row">
                <button
                  class="conduct-dir-btn conduct-dir-btn--neg"
                  :class="{ 'is-active': conductDir === '-1' }"
                  @click="conductDir = conductDir === '-1' ? null : '-1'; conductReason = null"
                >− NEG</button>
                <button
                  class="conduct-dir-btn conduct-dir-btn--pos"
                  :class="{ 'is-active': conductDir === '+1' }"
                  @click="conductDir = conductDir === '+1' ? null : '+1'; conductReason = null"
                >+ POS</button>
              </div>
              ...
              <!-- Reason -->
              <div v-if="conductDir" class="conduct-reason-grid">
                <button
                  v-for="r in CONDUCT_REASONS"
                  :key="r.key"
                  class="conduct-reason-btn"
                  :class="{
                    'is-selected': conductReason === r.key,
                    'is-neg': conductDir === '-1',
                    'is-pos': conductDir === '+1',
                  }"
                  @click="conductReason = r.key"
                >{{ conductDir === '-1' ? r.negLabel : r.label }}</button>
              </div>
              ...
              <input
                ...
                class="conduct-note-input"
              />

              <!-- Date (retroactive) -->
              <div class="conduct-date-row">
                <label class="conduct-date-label">DATE</label>
                <input
                  v-model="conductDate"
                  type="date"
                  class="conduct-date-input"
                  :max="todayISO()"
                />
              </div>
            </div>
            ...
            <div v-if="selectedUids.size > 0" class="conduct-selected-list">
              <div class="roster-group-label">SELECTED</div>
              <div
                v-for="student in periodStudents.filter(s => selectedUids.has(s.uid))"
                :key="student.uid"
                class="roster-item conduct-selected-item"
                @click="toggleSeatSelection(student.uid)"
              >
                <span class="roster-dot roster-dot--selected"></span>
                <span class="roster-name">{{ student.displayName }}</span>
                <span class="conduct-deselect">✕</span>
              </div>
            </div>
          </template>
...
.front-banner--bottom { margin-top: 0.25rem; }
...
.seat.is-selected {
  ...
  box-shadow: 0 0 0.6rem rgba(255, 153, 0, 0.3) !important;
}
...
.close-period-btn:hover:not(:disabled) {
  background: rgba(255, 153, 0, 0.2);
  border-color: #ff9900;
  color: #ff9900;
}
.close-period-btn:disabled { opacity: 0.3; cursor: not-allowed; }
...
.close-period-go-btn:hover:not(:disabled) {
  background: rgba(255, 153, 0, 0.35);
  box-shadow: 0 0 0.6rem rgba(255, 153, 0, 0.4);
}
.close-period-go-btn:disabled { opacity: 0.4; cursor: not-allowed; }
...
.conduct-btn:hover:not(:disabled) {
  ...
  box-shadow: 0 0 0.6rem rgba(255, 153, 0, 0.35);
}
.conduct-btn:disabled { opacity: 0.3; cursor: not-allowed; }
...
.seating-sidebar.is-conduct-mode {
...
.conduct-dir-btn:hover { color: #aaccdd; border-color: rgba(255,255,255,0.25); }

.conduct-dir-btn--neg.is-active {
  background: rgba(185, 74, 72, 0.15);
  border-color: var(--conduct-neg);
  color: var(--conduct-neg);
}
.conduct-dir-btn--pos.is-active {
  background: rgba(31, 120, 180, 0.15);
  border-color: var(--conduct-pos);
  color: var(--conduct-pos);
}
...
.conduct-reason-btn:hover { color: #aaccdd; border-color: rgba(255,255,255,0.22); }
.conduct-reason-btn.is-selected.is-neg {
  background: rgba(185, 74, 72, 0.15);
  border-color: var(--conduct-neg);
  color: var(--conduct-neg);
}
.conduct-reason-btn.is-selected.is-pos {
  background: rgba(31, 120, 180, 0.15);
  border-color: var(--conduct-pos);
  color: var(--conduct-pos);
}
...
.conduct-note-input:focus { outline: none; border-color: rgba(51, 102, 255, 0.45); }
.conduct-note-input::placeholder { color: #334455; }
...
.conduct-date-input:focus { outline: none; border-color: rgba(51,102,255,0.45); }

.conduct-selected-list {
  flex: 1;
  overflow-y: auto;
  padding: 0.25rem 0 0.5rem;
}

.conduct-selected-item {
  cursor: pointer;
  justify-content: space-between;
}
.conduct-selected-item:hover { background: rgba(255, 50, 50, 0.06); }

.roster-dot--selected { background: #ff9900; }
...
.conduct-selected-item:hover .conduct-deselect { color: #ff6644; }
...
.rotate-btn:hover:not(:disabled) {
  ...
  border-color: #9966ff;
  box-shadow: 0 0 0.6rem rgba(153, 102, 255, 0.3);
}

.rotate-btn:disabled { opacity: 0.3; cursor: not-allowed; }
