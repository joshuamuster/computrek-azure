...
/**
 * Returns a Record mapping systemKey → CSS filter string.
 * ShipDiagram applies these to colorize zones based on class health,
 * using the same grading color scale as AdminSeatingChart:
 *   ≥90 → blue   (#3399ff)  — all systems nominal
 *   ≥80 → green  (#00cc66)  — running well
 *   ≥70 → yellow (#ffcc00)  — minor fluctuations
 *   ≥60 → orange (#ff9900)  — systems degraded
 *    <60 → red    (#ff4422)  — critical failure
 * Zones with no graded data yet are omitted (keep original SVG colors).
 */
const zoneFilters = computed((): Record<string, string> => {
  ...
  for (const key of Object.keys(SYSTEMS)) {
    ...
    if (!periodId) { continue } // No period assigned — keep original SVG colors
    const doc = statusByPeriod.value.get(periodId)
    const hasData = !!doc && (doc.gradedCount > 0 || doc.overdueCount > 0)

    let filterId: string
    if      (!hasData)        filterId = 'zone-health-default'
    else if (doc!.health >= 90) filterId = 'zone-health-blue'
    else if (doc!.health >= 80) filterId = 'zone-health-green'
    else if (doc!.health >= 70) filterId = 'zone-health-yellow'
    else if (doc!.health >= 60) filterId = 'zone-health-orange'
    else                        filterId = 'zone-health-red'

    result[key] = `url(#${filterId})`
  }
  return result
})
