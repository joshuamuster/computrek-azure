...
function triggerGameOver(winner: PlayerRole) {
  ...
  if (isMultiplayer.value) {
    ...
    if (iWon) {
      ...
      scoreComposable.autoLogCompletion(scoreComposable.myHighScore.value ?? 0, 'loss'); // Battleship has no draws
    }
  } else {
...
watch(remoteRoom, (newRoom) => {
  if (!gameStartLogged.value && newRoom?.players?.white && newRoom?.players?.black) {
    ...
    logEvent('game_start', { gameId: 'battle_mutara_nebula' });
  }
...
onMounted(async () => {
  ...
  if (route.query.mode === 'online') {
    ...
    resetState();
    // Solo pass-and-play starts immediately
    logEvent('game_start', { gameId: 'battle_mutara_nebula' });
    gameStartLogged.value = true;
  }
