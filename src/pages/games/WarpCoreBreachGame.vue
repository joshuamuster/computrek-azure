<template>
  <div class="df-game-container" :class="{ show: isVisible }">
    ...
    <div v-if="showResumeModal" class="game-status-overlay confirmation">
      ...
      <p class="confirm-sub">
        Puzzle {{ resumeSaveData?.currentPuzzleId }} · {{ resumeSaveData?.filledCount ?? 0 }} pipe{{ (resumeSaveData?.filledCount ?? 0) === 1 ? '' : 's' }} placed
      </p>
...
const gameSave = useGameSave('warp_core_breach');
...
function onPlace(idx: number) {
  if (gameOver.value || showResumeModal.value) return;
  // Log game_start on first pipe placed
  if (!gameStartLogged.value) {
    gameStartLogged.value = true;
    logEvent('game_start', { gameId: 'warp_core_breach', difficulty: currentPuzzle.value?.difficulty ?? 'easy' });
  }
...
function endGame(won: boolean) {
  ...
  if (won) {
    ...
    // Submit score — only saves if it beats the personal best; also logs 'win'
    ...
    playSound(soundFail);
    // Log loss completion
    activeScores.value.autoLogCompletion(score.value, 'loss');
  }
}
...
function continueMission() {
  ...
  cells.value            = save.cells;
  queue.value            = save.queue;
  score.value            = save.score;
  filledCount.value      = save.filledCount;
  countdownLeft.value    = save.countdownLeft;
  countdownMs.value      = save.countdownMs ?? save.countdownLeft;
  flowIdx.value          = save.flowIdx;
  flowExitDir.value      = save.flowExitDir;
  isFlowing.value        = save.isFlowing;
  exitReached.value      = save.exitReached ?? false;
  gameOver.value         = false;
  victory.value          = false;
  gameStartLogged.value  = true; // already started before the save

  gameSave.clearSave();
  logEvent('game_continue', { gameId: 'warp_core_breach', difficulty: currentPuzzle.value?.difficulty ?? 'easy' });
...
function newMission() {
  ...
  gameSave.clearSave();
  gameStartLogged.value = false;
  // onMounted skipped the RAF when a save was found — start it now
  lastTs = 0;
  rafId = requestAnimationFrame(tick);
}
...
onMounted(() => {
  ...
  // Check for a saved game before starting the RAF loop
  const save = gameSave.loadSave();
  if (save) {
    resumeSaveData.value = save;
    showResumeModal.value = true;
    // Don't start the RAF loop yet — continueMission() or newMission() will start it
  } else {
    rafId = requestAnimationFrame(tick);
  }
...
onBeforeUnmount(() => {
  // Save if pipes have been placed and game isn't over
  if (!gameOver.value && cells.value.some(c => c.content?.kind === 'pipe')) {
    gameSave.saveGame({
      ...
      countdownLeft:    countdownLeft.value,
      countdownMs:      countdownMs.value,
      flowIdx:          flowIdx.value,
      flowExitDir:      flowExitDir.value,
      isFlowing:        isFlowing.value,
      exitReached:      exitReached.value,
    });
  }
  cancelAnimationFrame(rafId);
