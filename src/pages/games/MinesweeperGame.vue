<template>
  <div class="minesweeper-container" :class="{ show: isVisible }">
    ...
    <div v-if="showResumeModal" class="game-status-overlay resume-modal">
      ...
      <p class="confirm-sub">An unfinished simulation was found.</p>
...
const difficulties: Record<string, Difficulty> = {
  ...
  'red-alert': { name: 'Red Alert', rows: 16, cols: 30, mines: 92, safeZoneRadius: 1 }, // only the clicked cell
};
...
/** Formats a unix-ms timestamp as "May 23 at 2:30 PM" */
...
function continueMission() {
  ...
  // Restore difficulty first so board dimension expectations are consistent
  setDifficulty(save.difficulty as any);

  // Restore all game state
  board.value = save.board;
  timer.value = save.timer;
  minesRemaining.value = save.minesRemaining;
  gameMode.value = save.gameMode ?? 'reveal';
  gameState.value = 'playing';
  firstClick.value = false; // mines already placed in saved board

  // Clear the save — it will be re-written on next unmount if still in progress
  gameSave.clearSave();

  // Log the continuation
  logEvent('game_continue', { gameId: 'minesweeper', difficulty: save.difficulty });
...
function revealCell(r: number, c: number) {
  ...
  if (firstClick.value) {
    ...
    startTimer();
    logEvent('game_start', { gameId: 'minesweeper', difficulty: selectedDifficulty.value });
  }
...
function toggleFlag(r: number, c: number) {
  if (gameState.value !== 'playing' || showResumeModal.value || board.value[r][c].revealed) return;
  const cell = board.value[r][c];
...
function gameOver(won: boolean) {
  ...
  stopTimer();
  gameSave.clearSave(); // game ended — no need to keep a save
  if (!won) {
    ...
    // Log loss completion (wins are logged inside submitScore)
    activeScores.value.autoLogCompletion(timer.value, 'loss');
  } else {
    playSound(soundSuccess);
    // Submit time to Firestore — only saves if it's a personal best; also logs 'win'
...
function handleMouseUp(e: MouseEvent, r: number, c: number) {
  if (gameState.value !== 'playing' || showResumeModal.value) return;
...
/** Save current board if a game is actively in progress. */
function _maybeSaveGame() {
  if (gameState.value === 'playing' && !firstClick.value) {
    gameSave.saveGame({
      difficulty: selectedDifficulty.value,
      board: board.value,
      timer: timer.value,
      minesRemaining: minesRemaining.value,
...
onMounted(() => {
  ...
  // Check for a saved game from a previous session
  ...
  // Also save on hard refresh / tab close
  window.addEventListener('beforeunload', _maybeSaveGame);
});

onBeforeUnmount(() => {
  // Save before navigating away (Vue Router unmount path)
  _maybeSaveGame();
  stopTimer();
  // Clear dimming when leaving the game
  window.dispatchEvent(new CustomEvent('opening-intro-clear', { detail: true }));
  window.removeEventListener('beforeunload', _maybeSaveGame);
});
</script>

<style scoped>
/* game-logo-small and game-header layout shared via gameLanding.css,
   which is now imported globally in MainFrame.vue to prevent FOUC on refresh */
