<template>
  <div id="chrono2" class="chronobox">
      <div class="timer2-timer timer2-colors" :class="{ 'urgent-blink': !isNoSchool && missionClockSeconds > 0 && missionClockSeconds <= 120 }">
        <template v-if="isNoSchool"><span class="no-school-dash">– –</span></template>
        <template v-else><span v-if="clockChars.suppress < 1" class="t-d">{{ clockChars.chars[0] }}</span><span v-if="clockChars.suppress < 2" class="t-d">{{ clockChars.chars[1] }}</span><span v-if="clockChars.suppress < 3" class="t-d">{{ clockChars.chars[2] }}</span><span class="t-d">{{ clockChars.chars[3] }}</span>{{ clockChars.chars[4] }}<span class="t-d">{{ clockChars.chars[5] }}</span><span class="t-d">{{ clockChars.chars[6] }}</span></template>
      </div>
...
// No countdown on weekends / holidays — scheduleType is null
const isNoSchool = computed(() => !scheduleType.value)
