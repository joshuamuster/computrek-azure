...
import shipSrc from '@/assets/images/hud/Vector-Ship.svg'
...
const identityLabel = computed(() => {
  const periodShort = selectedPeriodId.value?.replace('period-', 'P')
  if (isAdmin.value) return periodShort ? `Admin, ${periodShort}` : 'Admin'
  if (isStaff.value) return periodShort ? `Staff, ${periodShort}` : 'Staff'
