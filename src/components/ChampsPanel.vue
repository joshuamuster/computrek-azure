<template>
  <div class="champs-wrapper" :class="{ 'champs-wrapper--collapsed': !isExpanded }">

    <!-- ── Collapsed mini strip (cadets, not broadcasting) ─────────────── -->
    <div v-if="!isExpanded" class="champs-mini" @click="userExpanded = true">
      <div class="champs-mini-label">☰</div>

      <div class="champs-mini-row">
        <div class="champs-mini-header">
          <span class="champs-mini-letter color-c">C</span>
          <span class="champs-mini-score color-c">{{ activeLevels.conversation }}</span>
        </div>
        <div class="champs-mini-boxes">
          <div class="champs-mini-box" :class="{ 'active-c': activeLevels.conversation >= 1 }"></div>
          <div class="champs-mini-box" :class="{ 'active-c': activeLevels.conversation >= 2 }"></div>
          <div class="champs-mini-box" :class="{ 'active-c': activeLevels.conversation >= 3 }"></div>
        </div>
      </div>

      <div class="champs-mini-row">
        <div class="champs-mini-header">
          <span class="champs-mini-letter color-h">H</span>
          <span class="champs-mini-score color-h">{{ activeLevels.help }}</span>
        </div>
        <div class="champs-mini-boxes">
          <div class="champs-mini-box" :class="{ 'active-h': activeLevels.help >= 1 }"></div>
          <div class="champs-mini-box" :class="{ 'active-h': activeLevels.help >= 2 }"></div>
          <div class="champs-mini-box" :class="{ 'active-h': activeLevels.help >= 3 }"></div>
        </div>
      </div>

      <div class="champs-mini-row">
        <div class="champs-mini-header">
          <span class="champs-mini-letter color-a">A</span>
          <span class="champs-mini-score color-a">{{ activeLevels.activity }}</span>
        </div>
        <div class="champs-mini-boxes">
          <div class="champs-mini-box" :class="{ 'active-a': activeLevels.activity >= 1 }"></div>
          <div class="champs-mini-box" :class="{ 'active-a': activeLevels.activity >= 2 }"></div>
          <div class="champs-mini-box" :class="{ 'active-a': activeLevels.activity >= 3 }"></div>
        </div>
      </div>

      <div class="champs-mini-row">
        <div class="champs-mini-header">
          <span class="champs-mini-letter color-m">M</span>
          <span class="champs-mini-score color-m">{{ activeLevels.movement }}</span>
        </div>
        <div class="champs-mini-boxes">
          <div class="champs-mini-box" :class="{ 'active-m': activeLevels.movement >= 1 }"></div>
          <div class="champs-mini-box" :class="{ 'active-m': activeLevels.movement >= 2 }"></div>
          <div class="champs-mini-box" :class="{ 'active-m': activeLevels.movement >= 3 }"></div>
        </div>
      </div>

      <div class="champs-mini-row">
        <div class="champs-mini-header">
          <span class="champs-mini-letter color-p">P</span>
          <span class="champs-mini-score color-p">{{ activeLevels.participation }}</span>
        </div>
        <div class="champs-mini-boxes">
          <div class="champs-mini-box" :class="{ 'active-p': activeLevels.participation >= 1 }"></div>
          <div class="champs-mini-box" :class="{ 'active-p': activeLevels.participation >= 2 }"></div>
          <div class="champs-mini-box" :class="{ 'active-p': activeLevels.participation >= 3 }"></div>
        </div>
      </div>

      <div class="champs-mini-row">
        <div class="champs-mini-header">
          <span class="champs-mini-letter color-s">S</span>
          <span class="champs-mini-score color-s">{{ activeLevels.success }}</span>
        </div>
        <div class="champs-mini-boxes">
          <div class="champs-mini-box" :class="{ 'active-s': activeLevels.success >= 1 }"></div>
          <div class="champs-mini-box" :class="{ 'active-s': activeLevels.success >= 2 }"></div>
          <div class="champs-mini-box" :class="{ 'active-s': activeLevels.success >= 3 }"></div>
        </div>
      </div>
    ...
    <!-- ── Full expanded panel ─────────────────────────────────────────── -->
    <div v-else class="champs-container" @mousemove="handleMouseMove" @click="isCadet && (userExpanded = false)">
      <div
        id="Champs-Title"
...
const { isCadet, isTeacher, isAdmin, emulatingEmail, userInfo } = useAuth();
const { selectedPeriodId } = usePeriodSelector();
const { isBroadcasting } = useBroadcast();

// ─── Collapse state ───────────────────────────────────────────────────────────
// Cadets default to collapsed; broadcast and non-cadets always expanded.
const userExpanded = ref(false);
const isExpanded = computed(() => isBroadcasting.value || !isCadet.value || userExpanded.value);
...
// Subscribe for cadets as soon as userInfo is available (or becomes available after mount).
// Using watch + immediate handles the case where initializeAuth() runs after ChampsPanel mounts.
watch(
  () => ({ email: userInfo.value?.teacherEmail, periodId: userInfo.value?.periodId }),
  ({ email, periodId }) => {
    if (!isCadet.value) return;
    subscribeToChamps(email, periodId);
  },
  { immediate: true, deep: false }
);

// Admin emulating a teacher: subscribe read-only to their CHAMPS state.
// Also reacts when the admin switches the period selector while emulating.
watch([emulatingEmail, selectedPeriodId], ([email, periodId]) => {
  if (!isAdmin.value) return;

  if (email && periodId) {
    // Subscribe to the emulated teacher's state for this period (read-only view)
    subscribeToChamps(email, periodId);
  } else {
    // Emulation stopped — unsubscribe and restore admin's own local scratch state
    if (unsubscribeSnapshot) { unsubscribeSnapshot(); unsubscribeSnapshot = null; }
    Object.assign(activeLevels, loadFromLocalStorage());
  }
}, { immediate: true });
...
const handleTitleClick = async () => {
  if (isCadet.value) { userExpanded.value = false; return; } // collapse back to mini
  if (!isTeacher.value) return; // admins, audit → no-op
...
const handleMouseMove = (e) => {
  ...
  if (item) {
    // Store viewport-relative coords so the fixed-position tooltip can escape
    // the overflow:hidden clipping on .champs-wrapper / .champs-container.
...
<style scoped>
  ...
  .nav-desc {
    position: fixed;
    left: calc(var(--mouse-x, 9999px) + 0.9375rem);
    top: var(--mouse-y, 50%);
  ...
  /* ── Wrapper — controls width transition ──────────────────────────────── */
  .champs-wrapper {
    flex-shrink: 0;
    width: 13rem;
    height: 100%;
  ...
  .champs-mini {
    ...
    cursor: pointer;
    border: 0.25rem solid var(--champsdefault);
    border-radius: 0.5rem;
    background: linear-gradient(to bottom, rgba(100,100,100,0.2), rgba(0,0,0,0.4));
    transition: background 0.2s;
    box-sizing: border-box;
  }

  .champs-mini:hover {
    background: linear-gradient(to bottom, rgba(100,100,100,0.35), rgba(0,0,0,0.55));
  }
  ...
  /* Each row = header (letter+number) stacked above boxes */
  .champs-mini-row {
    display: flex;
    flex-direction: column;
    align-items: center;
  ...
  /* Letter and number side by side */
  .champs-mini-header {
    display: flex;
    flex-direction: row;
    font-size: 2rem;
  ...
  .champs-mini-box.active-c { background: var(--champsred); }
  .champs-mini-box.active-h { background: var(--champsgold); }
  .champs-mini-box.active-a { background: var(--champsyellow); }
  .champs-mini-box.active-m { background: var(--champsgreen); }
  .champs-mini-box.active-p { background: var(--champsblue); }
  .champs-mini-box.active-s { background: var(--champspurple); }
