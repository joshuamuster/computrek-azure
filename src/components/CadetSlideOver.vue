...
import { useConductLog, CONDUCT_REASONS } from '@/composables/useConductLog'
...
// The 4 categories — imported from useConductLog so AdminSeatingChart shares the same list
const REASONS = CONDUCT_REASONS
type ReasonKey = typeof CONDUCT_REASONS[number]['key']
    ...
    <div v-if="student" class="slideover-panel" role="dialog" :aria-label="`${student.displayName} — conduct panel`">
      ...
      <div class="entries-section">
        ...
        <div v-else class="entries-list">
          ...
          >
            <span class="entry-delta" :style="{ color: e.scoreDelta > 0 ? CONDUCT_COLORS.pos : CONDUCT_COLORS.neg }">
              {{ e.scoreDelta > 0 ? '+1' : '−1' }}
            </span>
...
.cs-btn--down.cs-btn--active {
  ...
  border-color: var(--conduct-neg); color: var(--conduct-neg);
}
.cs-btn--up:not(:disabled):hover,
.cs-btn--up.cs-btn--active {
  background: rgba(31, 120, 180, 0.18);
  border-color: var(--conduct-pos); color: var(--conduct-pos);
}
...
.submit-btn--neg:not(:disabled):not(.submit-btn--dim):not(.submit-btn--saved) {
  border-color: var(--conduct-neg); color: var(--conduct-neg); background: rgba(185,74,72,0.08);
}
.submit-btn--pos:not(:disabled):not(.submit-btn--dim):not(.submit-btn--saved) {
  border-color: var(--conduct-pos); color: var(--conduct-pos); background: rgba(31,120,180,0.08);
}
.submit-btn--saved {
  border-color: var(--conduct-pos); color: var(--conduct-pos); background: rgba(31,120,180,0.12);
}
...
.entry-row--neg { border-left-color: var(--conduct-neg); }
.entry-row--pos { border-left-color: var(--conduct-pos); }
