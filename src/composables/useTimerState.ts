...
// Stored when subscribing so the expiry tick can clean up the Firestore doc.
let timerContext: { email: string; periodId: string } | null = null
...
function startTick() {
  ...
  tickInterval = setInterval(() => {
    ...
    if (rem <= 0) {
      ...
      stopTick()
      playAlarmSound()
      // Write isRunning: false so future subscribers don't see a stale expired doc
      if (ctx) {
        writeTimerDoc(ctx.email, ctx.periodId, { isRunning: false, endsAt: null, pausedAt: 0 })
          .catch(e => console.warn('[useTimerState] expiry cleanup write:', e))
      }
    }
...
function applyRemoteState(data: Record<string, any>) {
  if (data.isRunning && data.endsAt) {
    ...
    const hadActiveTick = tickInterval !== null  // was a local tick already running?
    endsAtMs = ms
    ...
    if (rem > 0) startTick()
      ...
      stopTick()
      // Only alarm if we were already locally tracking this countdown — not on a
      // fresh subscription to an already-expired doc (e.g. switching periods on
      // the Reports page while old timer docs still have isRunning: true).
      if (hadActiveTick) playAlarmSound()
    }
  } else {
    endsAtMs = null
    stopTick()
...
function subscribeToTimerDoc(teacherEmail: string, periodId: string) {
  ...
  timerContext = { email: teacherEmail, periodId }
...
function playAlarmSound() {
  const { currentBlock } = useScheduleManager()
  if (currentBlock.value?.silent) return
