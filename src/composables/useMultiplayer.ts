...
export function useMultiplayer() {
  ...
  const createOnlineGame = async (
    ...
    error.value = null;
    try {
      const { joinCode: code, roomId: createdId } = await createGame(gameType, initialBoardState, initialTurn, variant);

      roomId.value = createdId;
      joinCode.value = code;
      playerColor.value = 'white';
      isMultiplayer.value = true;
      
      startListening(createdId);
  ...
  const submitMove = async (move: any, nextState: string, nextTurn: string, pocket?: any) => {
    if (!roomId.value || !isMultiplayer.value) return;
    try {
      await submitOnlineMove(roomId.value, move, nextState, nextTurn, pocket);
    } catch (err: any) {
  ...
  const resetGame = async (initialBoardState: string, initialTurn: string = 'w', startFen?: string, resetPocket?: boolean) => {
    if (!roomId.value || !isMultiplayer.value) return;
    try {
      await resetOnlineGame(roomId.value, initialBoardState, initialTurn, startFen, resetPocket);
    } catch (err: any) {
