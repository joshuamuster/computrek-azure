...
async function ensureDoc(key: string, teacherEmail: string, periodId: string, schoolYearId: string) {
  await setDoc(
    ...
    { teacherEmail, periodId, schoolYearId, assignedCount: 0, submittedCount: 0, gradedCount: 0, onTimeCount: 0 },
    { merge: true },
  )
}
 ...
 * Increments submittedCount by 1, and onTimeCount by 1 if the submission is on time.
 *
 * @param isOnTime  Pass true when submittedAt ≤ effectiveDueDate; false otherwise.
 *                  Defaults to false so old call sites without the argument are safe.
 */
export async function onSubmitted(
  ...
  try {
    ...
    await updateDoc(doc(db, 'periodStats', key), {
      ...
      ...(isOnTime ? { onTimeCount: increment(1) } : {}),
      lastUpdated:    serverTimestamp(),
    })
  } catch (e) {
