<template>
  <section class="adventure-page">
    ...
    <div v-else class="grading-container">
      ...
      <div v-if="activeTab === 'assignment'" class="tab-panel">
        ...
        <!-- Submissions area (only when a period is selected) -->
        <template v-if="selectedPeriodId">
          ...
          <template v-else>
              ...
              <span class="col-actions">
                <template v-if="!isAudit">
                  ...
                  <button
                      ...
                      :title="`Delete ${studentName(sub.studentId)}'s submission`"
            ...
            <div v-if="editingGrade === sub.id" class="grade-row">
              ...
              >Stage → Return</button>
              <button class="action-btn cancel-btn" @click="cancelGrade">Cancel</button>
            </div>
            ...
            <div v-if="editingExtend === sub.id" class="extend-row">
              ...
              >Clear</button>
              <button class="action-btn cancel-btn" @click="cancelExtend">Cancel</button>
            </div>
          ...
          </template><!-- end v-else (not loading/error) -->
        </template><!-- end v-if="selectedPeriodId" -->
  ...
  <Teleport to="body">
    <div v-if="deleteModalSub" class="del-sub-backdrop" @click.self="deleteModalSub = null">
      <div class="del-sub-modal" role="dialog" aria-modal="true">
        <div class="del-sub-header">DELETE SUBMISSION</div>
        <div class="del-sub-body">
          <p class="del-sub-student">{{ studentName(deleteModalSub.studentId) }}</p>
          <p class="del-sub-mission">{{ assignmentTitle(deleteModalSub.assignmentId) }}</p>
          <div class="del-sub-warning">
            This permanently removes the student's submission record. The assignment will
            still exist — the student will appear as unsubmitted. This cannot be undone.
          </div>
        </div>
        <div class="del-sub-footer">
          <button class="action-btn cancel-btn" :disabled="isDeleting" @click="deleteModalSub = null">Cancel</button>
          <button class="action-btn del-sub-confirm-btn" :disabled="isDeleting" @click="doDeleteSub">
...
const {
  ...
  isLoading, error,
  fetchAllByStudent, fetchAllByPeriod, fetchNeedingGradingByPeriod, fetchStudents,
  gradeSubmission, returnSubmission, ungradeSubmission, setDueDateOverride,
  markComponentDone, deleteSubmission,
} = useSubmissions()
...
const confirmationQueue  = ref<Submission[]>([])  // assigned + manual items, awaiting teacher confirmation
...
async function loadGradeQueue() {
  ...
  // Fetch ALL submissions for the period so we can split into grade queue + confirmation queue
  await fetchAllByPeriod(selectedPeriodId.value)
  const all = rawSubmissions.value

  // Grade queue: already submitted or returned
  submissions.value = all.filter(s => s.status === 'submitted' || s.status === 'returned')

  // Confirmation queue: assigned + mission has at least one manual delivery item
...
const markingComponent = ref<string | null>(null)  // "subId_itemIndex" while in-flight

/** Returns the manual delivery items for a given submission's mission, with index attached. */
...
/** True when all manual items for this submission have been confirmed. */
...
/** Human-readable label for a delivery item, used in confirmation checkboxes. */
...
async function toggleComponent(sub: Submission, itemIndex: number) {
  ...
  if (ok) {
    // Update local state in the confirmation queue entry
    ...
    if (entry) {
      ...
      // If all manual items are now done, promote to the grade queue
...
const deleteModalSub = ref<Submission | null>(null)
const isDeleting     = ref(false)

function confirmDeleteSub(sub: Submission) {
  deleteModalSub.value = sub
  cancelGrade()
  cancelExtend()
}

async function doDeleteSub() {
  if (!deleteModalSub.value) return
  isDeleting.value = true
  const ok = await deleteSubmission(deleteModalSub.value.id)
  isDeleting.value = false
  if (ok) {
    submissions.value = submissions.value.filter(s => s.id !== deleteModalSub.value!.id)
    deleteModalSub.value = null
  }
}
...
/* Cancel button — visible but unobtrusive */
.cancel-btn {
  border: 0.0625rem solid rgba(153, 204, 255, 0.3);
  color: #7799bb;
}
...
/* Delete submission button (in col-actions row) */
.delete-sub-btn {
  border: 0.0625rem solid rgba(244, 67, 54, 0.3);
  color: #cc4433;
}
.delete-sub-btn:hover:not(:disabled) {
...
.del-sub-body { padding: 1.1rem 1.25rem; }

.del-sub-student {
  font-family: 'Antonio', 'Arial Narrow', sans-serif;
  font-size: 1rem; letter-spacing: 0.06em;
  color: #cce0ff; margin: 0 0 0.15rem;
}

.del-sub-mission {
  font-family: 'Antonio', 'Arial Narrow', sans-serif;
  font-size: 0.78rem; letter-spacing: 0.05em;
  color: #556677; margin: 0 0 0.85rem;
}
...
.del-sub-warning {
  ...
  border-radius: 0.3rem;
  padding: 0.6rem 0.75rem;
  font-size: 0.78rem; color: #99aaaa; line-height: 1.5;
}
...
.del-sub-confirm-btn {
  border: 0.0625rem solid rgba(244, 67, 54, 0.5);
  color: #f44336;
}
.del-sub-confirm-btn:hover:not(:disabled) {
  background: rgba(244, 67, 54, 0.12);
  border-color: #f44336;
}
</style>