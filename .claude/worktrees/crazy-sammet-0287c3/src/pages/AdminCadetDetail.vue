...
const activeTab = ref<'missions' | 'typing' | 'conduct'>('missions')
...
const sortedSubs = computed(() =>
  [...submissions.value].sort((a, b) => {
    const order = { submitted: 0, returned: 1, assigned: 2, graded: 3 }
        ...
        <template v-else>
          ...
          <div v-else class="conduct-entry-list">
            ...
            >
              <span
                class="ce-delta"
                :style="{ color: e.scoreDelta > 0 ? '#4caf50' : '#f44336' }"
...
.conduct-score-block .cs-btn--down:not(:disabled):hover {
  background: rgba(244, 67, 54, 0.2); border-color: #f44336; color: #f44336;
}
...
.csh-btn--down:not(:disabled):hover { background: rgba(244,67,54,0.2); border-color: #f44336; color: #f44336; }
.csh-btn--up:not(:disabled):hover   { background: rgba(76,175,80,0.2); border-color: #4caf50; color: #4caf50; }
...
.ce--pos { border-left: 3px solid #4caf50; }
.ce--neg { border-left: 3px solid #f44336; }
