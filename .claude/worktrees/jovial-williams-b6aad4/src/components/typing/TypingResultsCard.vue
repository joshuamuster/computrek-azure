<template>
  <div class="results-card" :class="{ show: visible }">
    ...
    <!-- Pass/Fail banner (lessons & tutorials) -->
    ...
    <div class="results-actions">
      <button class="pill" :class="backHighlighted ? 'cta' : 'secondary'" @click="$emit('back')">Back to Menu</button>
      <button class="pill" :class="tryAgainHighlighted ? 'cta' : 'secondary'" @click="$emit('tryAgain')">Try Again</button>
      <button v-if="nextLessonId" class="pill" :class="nextHighlighted ? 'cta' : 'secondary'" @click="$emit('next')">Next Lesson</button>
    </div>
...
const props = defineProps<{
  ...
  nextLessonIsLockedTest?: boolean          // true if next lesson is a test not yet unlocked
}>()
...
// didPass: true if lesson passed or test scored ≥ 95
const didPass = computed(() => {
  if (props.passed !== null && props.passed !== undefined) return props.passed
  if (props.score  !== null && props.score  !== undefined) return props.score >= 95
  return false
})

const hasNextAvailable = computed(() =>
  !!props.nextLessonId && !props.nextLessonIsLockedTest
)

const tryAgainHighlighted = computed(() => !didPass.value)
const nextHighlighted     = computed(() => didPass.value && hasNextAvailable.value)
