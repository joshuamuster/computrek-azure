  ...
  <section class="adventure-page">
      ...
      <div v-if="isAdmin" class="panel">
        ...
        <div class="panel-body">
          <div class="form-row">
            ...
            <!-- Cadet: teacher selector (pick teacher first — unlocks period dropdown) -->
            <div v-if="newUser.role === 'cadet'" class="field">
    ...
    <div v-if="pwResetTarget" class="modal-overlay" @click.self="closePwResetModal">
      <div class="modal modal--reset">
        ...
        <!-- After reset: show the temp password -->
        <template v-else>
          <div class="modal-title modal-title--reset">PASSWORD RESET</div>
          <p class="modal-body">
            <strong>{{ pwResetResult.displayName }}</strong>'s temporary password is:
          </p>
          <div class="temp-password-display">{{ pwResetResult.defaultPassword }}</div>
          <p class="modal-body" style="margin-top: 0.75rem; font-size: 0.88rem;">
            Give them this password and tell them they'll be prompted to set a new one after logging in.
          </p>
          <div class="modal-actions">
...
const addUser = async () => {
  ...
  if (isCadet) {
    ...
    try {
      ...
      // Save to Firestore
      await setDoc(doc(db, 'approvedUsers', cadetEmail), {
      ...
      newUser.value = { displayName: '', email: '', username: '', password: '', periodId: '', teacherEmail: '', periodIds: [], role: 'cadet' }
      await fetchUsers()
    } catch (e) {
      console.error(e)
...
const editForm     = ref({ displayName: '', role: '', periodId: '', teacherEmail: '', periodIds: [], requiresPasswordChange: false })
