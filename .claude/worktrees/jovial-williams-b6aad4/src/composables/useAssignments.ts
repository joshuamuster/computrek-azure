...
export function useAssignments() {
  ...
  const deployMission = async (draft: DeployDraft): Promise<string | null> => {
    ...
    try {
      ...
      // Collect gameIds from auto delivery items so they can be stamped on submission
      // docs as a top-level queryable field. useGameScores uses this to auto-submit.
      const autoGameId = draft.deliveryItems.find(
