...
export function useGameScores(gameId, { lowerIsBetter = false } = {}) {
	...
	/**
	 * Finds any submission docs assigned to this student for this gameId that are
	 * still in 'assigned' status and marks them submitted. Called automatically
	 * by submitScore and incrementScore so teachers don't have to mark games done.
	 */
	const autoLogCompletion = async (score) => {
		const uid          = userInfo.value?.uid
