...
export interface TypingResultDoc extends TypingResult {
  ...
  lessonType?:    string    // 'tutorial' | 'lesson' | 'test'
...
export function useTypingResults() {
  ...
  /** Get the current user's best WPM for one specific lesson. */
  const fetchMyBestWpmForLesson = async (lessonId: string): Promise<number | null> => {
    const uid = userInfo.value?.uid
    if (!uid) return null
    try {
      const q = query(
      ...
      const snap = await getDocs(q)
      return snap.empty ? null : (snap.docs[0].data().wpm ?? null)
    } catch (e) {
