 ...
 * Returns a reactive Map<periodId, ShipStatusDoc> that Systems.vue can use
 * to drive visual health and modal data.
 *
 * Automatically unsubscribes when the calling component unmounts.
 */
export function useShipStatus(): { statusByPeriod: Ref<Map<string, ShipStatusDoc>> } {
  const statusByPeriod = ref(new Map<string, ShipStatusDoc>())

  const unsub = onSnapshot(collection(db, 'shipStatus'), snap => {
