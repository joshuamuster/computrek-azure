<template>
  <div class="game-landing-container" :class="{ 'fade-out': hasClicked }">
    <div class="intro-view">
      ...
      <div class="logo-row">
        <img :src="picardManeuverLogo" alt="Picard Maneuver" class="game-logo" />
      </div>
      ...
      <div class="landing-layout" :class="{ 'dimmed': hasClicked }">
        ...
        <div class="landing-left">
          ...
          <div class="mission-subheading">
            <h3>The C.S.S. Curiosity has engaged in a tactical exercise simulating the legendary Maneuver.</h3>
            <p>Out‑position your opponent on a shifting tactical grid, seizing control of the
              field through superior strategy and calculated precision.</p>
          ...
          <div class="mission-briefing">
            <p>The Picard Maneuver challenges cadets to think several moves ahead as they attempt to dominate the
              tactical grid by surrounding and converting opposing pieces. Each placement alters the balance of
              power, forcing both sides to constantly reassess the state of the board. Through careful observation, strategic positioning, and adaptive planning, cadets learn to anticipate opponent responses, exploit openings, and secure victory through superior tactical insight.</p>
...
import { getInitialBoard, boardToString } from '@/components/games/PicardManeuver/engine';
...
const boardComposable = useGameScores('picard_maneuver', SCORE_OPTIONS);
...
const gameInfo = gameData.picardManeuver;
...
onBeforeUnmount(() => {
  if (!hasClicked.value) {
    resetState(); window.dispatchEvent(new CustomEvent("opening-intro-clear", { detail: true }));
  }
});
...
async function handleCreate() {
  ...
  // In Picard Maneuver, Black (2) usually moves first.
  await createOnlineGame('picard-maneuver', boardStr, 'black');
}

async function handleJoin() {
  if (!inputCode.value) return;
  await joinOnlineGame('picard-maneuver', inputCode.value.toUpperCase());
  if (!multiError.value) {
    startTransition('online');
  }
}
...
async function launchLocal() {
  ...
  router.push('/games/picard-maneuver/play');
}
...
async function startTransition() {
  ...
  window.dispatchEvent(new CustomEvent('opening-intro-dim', { detail: true }));
  await new Promise(resolve => setTimeout(resolve, 1500));
  router.push('/games/picard-maneuver/play?mode=online');
}
</script>
