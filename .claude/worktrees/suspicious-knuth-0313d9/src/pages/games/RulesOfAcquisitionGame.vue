    ...
    if (!pitsMatch) {
      ...
      if (isGameOver && !showGameOver.value) {
        ...
        if (winner !== 'tie') {
          ...
          if (winner === myPlayer) scoreComposable.incrementScore();
        }
      }
    }
  }

  if (newRoom?.status === 'playing' && state.value.gameOver && !showConfirm.value) {
...
function applyMove(pitIndex: number, isLocal: boolean) {
  ...
  if (state.value.gameOver) {
    setTimeout(() => {
      ...
      playSound(soundSuccess);
      // Submit score if local player won
      if (isMultiplayer.value && state.value.winner !== 'tie') {
        const myPlayer = playerColor.value === 'white' ? Player.P1 : Player.P2;
        if (state.value.winner === myPlayer) scoreComposable.incrementScore();
      }
    }, 1000);
  } else {
    playSound(soundOk);
