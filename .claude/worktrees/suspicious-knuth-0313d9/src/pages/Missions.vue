<template>
  <div class="kanban-wrap">
    ...
    <div class="sticky-header">
      <div class="controls">
        <div v-if="loadingSheet" class="loading-status">Loading Missions Data...</div>
        <div v-else-if="fetchError" class="error-status" :title="fetchError">⚠️ Data Load Error</div>
...
// @ts-ignore
import scheduleCsvRawLocal from '@/assets/data/CS-Schedule-2025-2026.csv?raw';
// eslint-disable-next-line @typescript-eslint/ban-ts-comment
// @ts-ignore
...
const SHEET_URL        = 'https://docs.google.com/spreadsheets/d/1bSh_DAs_eMpK6y27Ws4NrtNajoqjuEPxZVDPVmwMc60/export?format=csv';
...
function buildScheduleMap(csvText: string): Map<string, { unit: string; lessonNum: string; lesson: string; points: string }> {
  ...
  const idxLessonNum   = header.indexOf('Lesson #');
  const idxLessonTitle = header.indexOf('Lesson Title');
  const idxPts         = header.indexOf('Pts.');
  const pad2 = (n: number) => String(n).padStart(2, '0');

  for (let i = 1; i < lines.length; i++) {
    const parts       = lines[i].split(',');
    const month       = (parts[idxMonth]       ?? '').trim();
    const dayStr      = (parts[idxDate]        ?? '').trim();
    const unit        = (parts[idxUnit]        ?? '').trim();
    const pts         = (parts[idxPts]         ?? '').trim();
    const lessonNum   = (parts[idxLessonNum]   ?? '').trim();
    const lessonTitle = (parts[idxLessonTitle] ?? '').trim();
    if (!month || !dayStr) continue;
    const mm = MONTH_NUM[month];
    const dayNum = parseInt(dayStr, 10);
    if (!mm || !Number.isFinite(dayNum)) continue;
    const year = mm >= 8 ? 2025 : 2026;
    const iso = `${year}-${pad2(mm)}-${pad2(dayNum)}`;
    const lesson = lessonTitle || '';
    map.set(iso, { unit, lessonNum, lesson, points: pts });
  }
  return map;
}
...
const scheduleCsvRaw  = ref<string>(String(scheduleCsvRawLocal || ''));
...
function initCards(schedMap: Map<string, { unit: string; lesson: string; points: string }>) {
...
const scheduleMap = computed(() => buildScheduleMap(scheduleCsvRaw.value));
...
onMounted(async () => {
  ...
  try {
    // 1. Fetch the CSV schedule (used to seed if no saved data exists)
    const resp = await fetch(SHEET_URL);
    if (!resp.ok) throw new Error(`HTTP ${resp.status}`);
    const csv = await resp.text();
    if (csv?.trim()) scheduleCsvRaw.value = csv;
  } catch (err: any) {
  ...
  // 2. Load card data: Firestore > localStorage > CSV seed
  try {
  ...
  const needsSave = migrateCardLessonNums(scheduleMap.value);
  if (needsSave) {
